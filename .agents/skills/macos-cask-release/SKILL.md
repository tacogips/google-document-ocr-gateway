...
brew install --cask google-document-ocr-gateway
```
...
swift run google-document-ocr-gateway --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/google-document-ocr-gateway-<version>-darwin-arm64.dmg
dist/homebrew-cask/google-document-ocr-gateway-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-document-ocr-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-document-ocr-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-document-ocr-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-document-ocr-gateway-${version}-darwin-x64.dmg"
```
...
DMGs to `tacogips/google-document-ocr-gateway`, and renders
`../homebrew-tap/Casks/google-document-ocr-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/google-document-ocr-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release google-document-ocr-gateway <version>"
...
```bash
brew fetch --cask tacogips/tap/google-document-ocr-gateway
HOMEBREW_NO_GITHUB_API=1 brew audit --cask tacogips/tap/google-document-ocr-gateway
```
...
verify `api/cask/google-document-ocr-gateway.json` from GitHub
