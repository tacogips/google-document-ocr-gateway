...
```bash
brew tap user/tap
brew install --cask google-document-ocr
```
...
swift run google-document-ocr --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/google-document-ocr-<version>-darwin-arm64.dmg
dist/homebrew-cask/google-document-ocr-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-document-ocr-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-document-ocr-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-document-ocr-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-document-ocr-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/google-document-ocr.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/google-document-ocr.rb README.md
git diff --staged --stat
git commit -m "chore: release google-document-ocr <version>"
...
```bash
brew fetch --cask user/tap/google-document-ocr
HOMEBREW_NO_GITHUB_API=1 brew audit --cask user/tap/google-document-ocr
```
...
verify `api/cask/google-document-ocr.json` from GitHub
